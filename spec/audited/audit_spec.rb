...
describe Audited::Audit do
  ...
  it "should set the version number on create" do
    ...
    expect(Audited::Audit.where(
      ...
      auditable_id: user.id).last.version).to(eq(3))
  end
  ...
  describe "reconstruct_attributes" do
    it "should work with the old way of storing just the new value" do
      audits = Audited::Audit.reconstruct_attributes([
        Audited::Audit.new(audited_changes: { "attribute" => "value" }),
      ])
  ...
  describe "as_user" do
    ...
    if ActiveRecord::Base.connection.adapter_name != "SQLite"
      it "should be thread safe" do
        ...
        t2 = Thread.new do
          Audited::Audit.as_user(user.name) do
            expect(Models::ActiveRecord::Company.create(name: "The Competing Auditors, LLC").audits.first.username)
              .to(eq(user.name))
            sleep(0.5)
          end
        end
