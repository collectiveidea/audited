...
describe Audited::Auditor do
 ...
 [ "favourite_device", "created_at", "updated_at", "password" ].include?(col) }).to(eq(false))
      ensure
  ...
  if ActiveRecord::VERSION::MAJOR >= 6
    describe "on touch" do
      ...
      it "should store the changed attributes" do
        ...
        expect(Time.parse(@user.audits.last.audited_changes["suspended_at"][1].to_s))
      ...
      context "don't double audit" do
        ...
        context "after creating" do
          ...
          it "updating / touching parent object shouldn't save previous data" do
            ...
            expect(user.audits.last.action).to(eq("update"))
            expect(user.audits.last.audited_changes.keys).to(eq([ 'suspended_at' ]))
          end
          ...
          it "updating nested resource through parent while changing an enum on parent shouldn't double audit" do
            ...
            expect(user.audits.last.action).to(eq("update"))
            expect(user.audits.last.audited_changes.keys).to(eq([ 'status' ]))
          end
        end
