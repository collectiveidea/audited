...
Sometimes it is necessary to combine several audits somehow. You can use +as_transaction+ to set the same +transaction_id+ and +comment+ to all done audits in the block:

    Audit.as_transaction(transaction_id, comment) do
      # Perform changes on audited models
    end
